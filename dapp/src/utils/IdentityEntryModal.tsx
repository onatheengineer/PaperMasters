import React, {FC, useRef, MouseEventHandler, useState, FormEvent} from 'react';
...
export const IdentityEntryModal:FC<identityEntryModalInterface> = ({text, isOpen, onClose, placeHolder, title, buttonText}) => {
    ...
    const navToProfilePageHandler = ()=>{
        navigate(`/identity/${initialRef.current?.value}`);
                    ...
                    <ModalBody pb={4}>
                        <FormControl mt={4}>
                            ...
                            <Input
                                ...
                                ref={initialRef} placeholder={placeHolder}
                            />
                        </FormControl>
                    </ModalBody>

                    <ModalFooter>
                        <Button bg={'pmpurple.8'} disabled={typedText.length <= 32} border={'1px solid'} borderColor={'pmpurple.13'}  color={'pmpurple.1'} mr={3} onClick={navToProfilePageHandler}>
                            {buttonText}
                        </Button>
                        <Button bg={'pmpurple.2'} border={'1px solid'} borderColor={'pmpurple.4'} color={'pmpurple.13'} mr={3} onClick={onClose}>Cancel</Button>
                    </ModalFooter>
                </ModalContent>
            </Modal>
        </Box>
