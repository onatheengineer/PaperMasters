...
describe("ContractTesting", () => {
    ...
    test('Network Supported', async () => {
        expect.assertions(Object.keys(MintABI.networks).length)
        const results = await Promise.all(Object.keys(MintABI.networks).map(async (chainId) => {
            const chainIdSupportedArr = chainIdNetworks.filter((el) => {
                return el.chainId === parseInt(chainId)
            });
    ...
    test('Contract Total Supply > 0', async () => {
        ...
        const results = await Promise.all(Object.keys(MintABI.networks).map(async (chainId) => {
            ...
            return await NFIContract.totalSupply().then((supply: BigNumber) => { return expect(supply.toNumber()).toBeGreaterThan(0)});
        }));
    });
    ...
    test('Identity 0 Defined', async () => {
        ...
        const results = await Promise.all(Object.keys(MintABI.networks).map(async (chainId) => {
            ...
            const ntw  = MintABI.networks as {[cid: string]: { address:string }};
            const NFIContract = new ethers.Contract(ntw[chainId].address, MintABI.abi, provider);
            return await NFIContract.tokenIDtoIdentityStruct(0).then((identity: any) => {
    ...
    test('Identity Fee 0.1 Eth', async () => {
        expect.assertions(Object.keys(MintABI.networks).length);
         const result = await Promise.all((Object.keys(MintABI.networks).map(async (chainId) => {
            const chainIdSupportedArr = chainIdNetworks.filter((el) => {
                return el.chainId === parseInt(chainId)
            });
            const network = chainIdSupportedArr[0].name.toLowerCase();
