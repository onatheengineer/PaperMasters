  ...
  test('Contract Total Supply > 0', async () => {
    ...
    const results = await Promise.all(
      Object.keys(MintABI.networks).map(async (chainId) => {
        ...
        return NFIContract.totalSupply().then((supply: BigNumber) => {
  ...
  test('Identity 0 Defined', async () => {
    ...
    const results = await Promise.all(
      Object.keys(MintABI.networks).map(async (chainId) => {
        ...
        return NFIContract.tokenIDtoIdentityStruct(0).then((identity: any) => {
  ...
  test('Identity Fee 0.1 Eth', async () => {
    ...
    const result = await Promise.all(
      Object.keys(MintABI.networks).map(async (chainId) => {
        ...
        return NFIContract.getIdentityFee().then((fee: BigNumber) => {
  ...
  test('isPaused', async () => {
    ...
    const result = await Promise.all(
      Object.keys(MintABI.networks).map(async (chainId) => {
        ...
        return NFIContract.paused().then((pause: boolean) => {
          return expect(pause).toBeFalsy();
        });
      }),
    );
  });
